...
SkinnedMesh2::SkinnedMesh2(std::istream& stream, const Skeleton& skeleton)
	:skeleton(skeleton)
{
...
void SkinnedMesh2::writeToCollada(xml_document<>& doc, xml_node<>* root) const
{
...
void SkinnedMesh2::readRigs(std::istream& stream, Lod& lod) const
{
...
void SkinnedMesh2::readMaterials(std::istream& stream, Lod& lod) const
{
...
char *SkinnedMesh2::writeGeometry(xml_document<>& doc, xml_node<>* libraryGeometries, const std::string& objectName, const Material& material) const
{
...
char* SkinnedMesh2::writeSkinController(rapidxml::xml_document<>& doc, rapidxml::xml_node<>* libraryControllers, const std::string& objectName, const Material& material, const Rig& rig, const char* meshId) const
{
...
void SkinnedMesh2::writeSceneObject(rapidxml::xml_document<>& doc, rapidxml::xml_node<>* visualScene, const std::string& objectName, const char* skinId) const
{
...
char* SkinnedMesh2::writeValueNtimes(rapidxml::xml_document<>& doc, size_t count, char* value) const
{
	std::stringstream ss;
...
std::pair<char*, size_t> SkinnedMesh2::computeIndices(rapidxml::xml_document<>& doc, const Material& material, size_t inputCount) const
{
	std::stringstream ss;
	size_t polycount = 0;
...
std::pair<char*, size_t> SkinnedMesh2::writeBoneNames(rapidxml::xml_document<>& doc, const Rig& rig) const
{
	std::stringstream ss;
	size_t count = 0;
...
std::pair<char*, size_t> SkinnedMesh2::writeBonePoses(rapidxml::xml_document<>& doc, const Rig& rig) const
{
	std::stringstream ss;
	size_t count = 0;
	for (const MeshBone& bone : rig.bones) {
...
std::pair<char*, size_t> SkinnedMesh2::writeVertexData(rapidxml::xml_document<>& doc, const Material& material, VertexAttrib::Usage usage) const
{
	size_t offset = -1;
	size_t elementCount;
...
size_t SkinnedMesh2::computeVertexWeights(const Material& material, std::vector<float>& weightData, std::vector<size_t>& indexData) const
{
